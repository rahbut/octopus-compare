...
- The tool runs **entirely in the browser with no server-side layer** — all API calls are made directly from the client, and no user data is proxied through or stored on any backend
- This is confirmed as viable: the Octopus API returns `Access-Control-Allow-Origin: *` on its responses, meaning direct browser requests are explicitly supported without any CORS proxy
...
- The **Ofgem Price Cap** reference data is bundled as a static JSON file (`ofgem-price-cap.json`), generated by a provided Python script and updated manually each quarter when Ofgem publishes new cap levels
